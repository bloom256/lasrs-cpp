...
class PointData
{
  public:
    ...
    size_t len() const
    {
        return lasrs_point_data_len(get());
    }
    ...
    point::Format format() const
    {
        return point::Format::from_c(lasrs_point_data_format(get()));
    }

    Vector<Transform> transforms() const
    {
        return detail::from_c(lasrs_point_data_transforms(get()));
    }

    std::span<const uint8_t> raw_bytes() const
    {
        return detail::to_span(lasrs_point_data_raw_bytes(get()));
    }

    size_t record_len() const
    {
        return lasrs_point_data_record_len(get());
    }
    ...
    std::vector<int32_t> x_raw() const
    {
        return column<int32_t>(lasrs_point_data_x_raw);
    }
    std::vector<int32_t> y_raw() const
    {
        return column<int32_t>(lasrs_point_data_y_raw);
    }
    std::vector<int32_t> z_raw() const
    {
        return column<int32_t>(lasrs_point_data_z_raw);
    }
    std::vector<double> x() const
    {
        return column<double>(lasrs_point_data_x);
    }
    std::vector<double> y() const
    {
        return column<double>(lasrs_point_data_y);
    }
    std::vector<double> z() const
    {
        return column<double>(lasrs_point_data_z);
    }
    std::vector<uint16_t> intensity() const
    {
        return column<uint16_t>(lasrs_point_data_intensity);
    }
    std::vector<uint8_t> classification() const
    {
        return column<uint8_t>(lasrs_point_data_classification);
    }
    std::vector<uint8_t> return_number() const
    {
        return column<uint8_t>(lasrs_point_data_return_number);
    }
    std::vector<uint8_t> number_of_returns() const
    {
        return column<uint8_t>(lasrs_point_data_number_of_returns);
    }
    std::vector<float> scan_angle_degrees() const
    {
        return column<float>(lasrs_point_data_scan_angle_degrees);
    }
    std::vector<uint8_t> user_data() const
    {
        return column<uint8_t>(lasrs_point_data_user_data);
    }
    std::vector<uint16_t> point_source_id() const
    {
        return column<uint16_t>(lasrs_point_data_point_source_id);
    }

    std::optional<std::vector<double>> gps_time() const
    {
        return optional_column<double>(lasrs_point_data_gps_time);
    }
    ...
    std::optional<std::vector<uint16_t>> nir() const
    {
        return optional_column<uint16_t>(lasrs_point_data_nir);
    }

    const LasrsPointData *c_ptr() const
    {
        return handle_.get();
    }
    LasrsPointData *c_ptr()
    {
        return handle_.get();
    }

    static PointData from_c(LasrsPointData *owned)
    {
        return PointData(owned);
    }
  ...
  private:
    ...
    const LasrsPointData *get() const
    {
        return handle_.get();
    }

    template <class T> std::vector<T> column(void (*fill)(const LasrsPointData *, T *)) const
    {
    ...
    template <class T> std::optional<std::vector<T>> optional_column(bool (*fill)(const LasrsPointData *, T *)) const
    {
        std::vector<T> values(len());
...
inline void Header::add_point_data(const PointData &points)
{
    lasrs_header_add_point_data(mut_ptr(), points.c_ptr());
}
