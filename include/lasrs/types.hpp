...
struct Version {
    ...
    constexpr bool requires_point_data_start_signature() const { return *this == Version(1, 0); }
...
struct Transform {
    ...
    constexpr double direct(int32_t n) const { return scale * static_cast<double>(n) + offset; }
...
constexpr bool is_standard(GpsTimeType type) { return type == GpsTimeType::Standard; }
...
constexpr bool is_user_definable(Classification c) { return static_cast<uint8_t>(c) >= 64; }
...
struct Format {
    ...
    explicit Format(uint8_t n) { *this = from_c(checked_new(n)); }
    ...
    uint16_t len() const { return lasrs_format_len(to_c()); }
...
struct Vlr {
    ...
    size_t len(bool is_extended) const { return data.size() + (is_extended ? 60 : 54); }
    bool is_empty() const { return data.empty(); }
    bool has_large_data() const { return data.size() > std::numeric_limits<uint16_t>::max(); }
    bool is_crs() const { return is_projection() && (record_id == 2112 || is_geotiff_record()); }
    bool is_wkt_crs() const { return is_projection() && record_id == 2112; }
    bool is_geotiff_crs() const { return is_projection() && is_geotiff_record(); }
...
private:
    ...
    bool is_geotiff_record() const { return record_id >= 34735 && record_id <= 34737; }
};
...
struct VoxelKey {
    ...
    constexpr VoxelKey parent() const { return {std::max(0, l - 1), x >> 1, y >> 1, z >> 1}; }
...
constexpr LasrsBounds to_c(const Bounds& b) { return {{b.min.x, b.min.y, b.min.z}, {b.max.x, b.max.y, b.max.z}}; }

constexpr Bounds from_c(const LasrsBounds& b) { return {{b.min.x, b.min.y, b.min.z}, {b.max.x, b.max.y, b.max.z}}; }

constexpr LasrsVersion to_c(Version v) { return {v.major, v.minor}; }

constexpr Version from_c(LasrsVersion v) { return {v.major, v.minor}; }

constexpr LasrsVoxelKey to_c(const copc::VoxelKey& k) { return {k.l, k.x, k.y, k.z}; }

constexpr copc::VoxelKey from_c(const LasrsVoxelKey& k) { return {k.l, k.x, k.y, k.z}; }

constexpr LasrsEntry to_c(const copc::Entry& e) { return {to_c(e.key), e.offset, e.byte_size, e.point_count}; }

constexpr copc::Entry from_c(const LasrsEntry& e) { return {from_c(e.key), e.offset, e.byte_size, e.point_count}; }
