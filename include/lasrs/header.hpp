...
public:
    ...
    uint16_t file_source_id() const { return lasrs_header_file_source_id(ptr_); }

    GpsTimeType gps_time_type() const { return static_cast<GpsTimeType>(lasrs_header_gps_time_type(ptr_)); }

    bool has_synthetic_return_numbers() const { return lasrs_header_has_synthetic_return_numbers(ptr_); }

    bool has_wkt_crs() const { return lasrs_header_has_wkt_crs(ptr_); }
    ...
    Version version() const { return detail::from_c(lasrs_header_version(ptr_)); }

    std::string_view system_identifier() const { return detail::to_string_view(lasrs_header_system_identifier(ptr_)); }
    ...
    std::span<const uint8_t> padding() const { return detail::to_span(lasrs_header_padding(ptr_)); }

    point::Format point_format() const { return point::Format::from_c(lasrs_header_point_format(ptr_)); }

    Vector<Transform> transforms() const { return detail::from_c(lasrs_header_transforms(ptr_)); }

    Bounds bounds() const { return detail::from_c(lasrs_header_bounds(ptr_)); }

    uint64_t number_of_points() const { return lasrs_header_number_of_points(ptr_); }
    ...
    std::span<const uint8_t> vlr_padding() const { return detail::to_span(lasrs_header_vlr_padding(ptr_)); }

    std::span<const uint8_t> point_padding() const { return detail::to_span(lasrs_header_point_padding(ptr_)); }
    ...
    bool has_crs_vlrs() const { return lasrs_header_has_crs_vlrs(ptr_); }

    void clear() { lasrs_header_clear(mut_ptr()); }
    ...
    void remove_crs_vlrs() { lasrs_header_remove_crs_vlrs(mut_ptr()); }
    ...
    bool operator==(const Header& other) const { return lasrs_header_eq(ptr_, other.ptr_); }

    const LasrsHeader* c_ptr() const { return ptr_; }

    static Header from_c(LasrsHeader* owned) { return Header(owned); }
...
private:
    ...
    LasrsHeader* mut_ptr() { return owned_.get(); }
...
struct Builder {
    ...
    explicit Builder(Version version) { assign(make_handle(lasrs_builder_from_version(detail::to_c(version)))); }
