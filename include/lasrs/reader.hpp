...
public:
    ...
    void seek(uint64_t position) { detail::check(lasrs_reader_seek(handle_.get(), position)); }
...
private:
    ...
    template <class Open>
    static Reader open(std::unique_ptr<detail::InputStream> input, Open open_reader) {
