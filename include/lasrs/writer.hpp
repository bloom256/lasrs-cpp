...
public:
    ...
    void close() { detail::check(lasrs_writer_close(handle_.get())); }

    Header header() const { return Header::from_c(lasrs_writer_header(handle_.get())); }
...
private:
    ...
    template <class Open>
    static Writer open(Open open_writer) {
