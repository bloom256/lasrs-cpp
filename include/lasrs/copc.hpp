...
{
  public:
    static LodSelection All()
    {
        return LodSelection({LASRS_LOD_SELECTION_KIND_ALL, 0.0, 0, 0});
    }
    static LodSelection Resolution(double resolution)
    {
        return LodSelection({LASRS_LOD_SELECTION_KIND_RESOLUTION, resolution, 0, 0});
    }
    static LodSelection Level(int32_t level)
    {
        return LodSelection({LASRS_LOD_SELECTION_KIND_LEVEL, 0.0, level, 0});
    }
...
{
  public:
    static BoundsSelection All()
    {
        return BoundsSelection({false, {}});
    }
    static BoundsSelection Within(const Bounds &bounds)
    {
        return BoundsSelection({true, detail::to_c(bounds)});
    }

    LasrsBoundsSelection to_c() const
    {
        return selection_;
    }
...
class CopcReader
{
  ...
  private:
    ...
    template <class Open> static CopcReader open(std::unique_ptr<detail::InputStream> input, Open open_reader)
    {
